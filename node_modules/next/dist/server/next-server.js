...
class NextNodeServer extends _baseServer.default {
    ...
    async handleMiddlewareRoute({ isDynamic , parsed , req , res  }) {
        var ref;
        if (!((ref = this.allRoutes) === null || ref === void 0 ? void 0 : ref.some((r)=>r.isMiddleware && (0, _isDynamic).isDynamicRoute(r.page) === isDynamic
        ))) {
            return {
                finished: false
            };
        ...
        for (const item of this.allRoutes || []){
            if (item.match(normalizedPathname)) {
                ...
                if ((0, _isDynamic).isDynamicRoute(item.page) !== isDynamic) continue;
    ...
    async runMiddleware(params) {
        ...
        if (await this.hasPage(normalizedPathname)) {
            ...
            const isApi = (0, _baseServer).isApiRoute(normalizedPathname);
            for (const dynamicRoute of this.dynamicRoutes){
                // The api route should not match with others
                if (isApi && !(0, _baseServer).isApiRoute(dynamicRoute.page)) continue;
